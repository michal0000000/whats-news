...
$.holdReady(true);
    setTimeout(function () {
        $.holdReady(false);
    }, 0); // set 3000 if you need to hold ready status of page during testing
...
$(document).ready(function () {
    ...
    $(window).scroll(function () {
        if($(window).scrollTop() >= $('#articles').offset().top + $('#articles').outerHeight() - window.innerHeight && end_pagination === false && block_request === false) {
            ...
            // Fetch biased/ubiased
            let unbiased = window.location.search
            if (unbiased){
              unbiased = unbiased.replace("?","").split("=")[1];
            }else{
              unbiased = false;
            }
...
function fetchNewArticles() {
  ...
  url = "new_posts/" + window.location.search
    ...
    if (data){
      ...
      $.each(data.data, function(index, article) {
        ...
        if (data.unbiased == false) {
          ...
          var articleSource = document.createElement("h3");
          articleSource.classList.add(["article-author"]);
          articleSource.setAttribute("href", article.source_link);
          articleSource.setAttribute("target", "_blank");
          articleSource.textContent = article.authors;
        ...
        // If unbiased mode
        console.log(data.unbiased)
        if (data.unbiased == true) {
          remove = newArticle.querySelector(".article-info")
          newArticle.removeChild(remove)
        }
        console.log(newArticle)
